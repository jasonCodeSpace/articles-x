...
function generateSlug(title: string): string {
  ...
  return title
    .toLowerCase()
    // Replace spaces and Japanese characters with hyphen
    .replace(/[\s\u3000-\u303f\u3040-\u309f\u30a0-\u30ff\uff00-\uffef]+/g, '-')
    // Remove special characters except hyphen, letters, numbers
    .replace(/[^\p{L}\p{N}-]/gu, '')
...
async function fixEmptySlugs() {
  console.log('Finding articles with empty slugs...\n')
  ...
  while (true) {
    ...
    console.log(`Found ${articles.length} articles with empty slugs`)
    ...
    offset += batchSize
    console.log(`\nProgress: ${totalUpdated} articles updated\n`)
